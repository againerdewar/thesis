...
More specifically, an $F_{k}$-structure which is invariant under some $\sigma \in \mathfrak{S}_{k}$ is in fact still just a $Y$-label and a set of $H_{k}$-structures which are invariant under that same $\sigma$.
...
\begin{proof}
  Note first that, if \eqref{eq:hfunc} held as an identity of $\mathfrak{S}_{k}$-species, we would instead have
  \begin{equation*}
