...
Then, for each label set $A$, the map $\action{\Gamma} \sbrac{A}: F \sbrac{A} \to \nicefrac{F}{\Gamma} \sbrac{A}$ is an injection, and the requirement that $\Gamma$ acts by natural transformations implies that $\action{\Gamma}: F \to \nicefrac{F}{\Gamma}$ is a natural transformation.
