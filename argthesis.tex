...
\begin{definition} 
  ...
  Finally, denote by $\catname{Bij}^{k}$ the category of finite $k$-sort sets with $k$-sort bijections.
\end{definition}

We can then define an extension of species to the context of $k$-sort sets:
\begin{definition}
  \label{def:multisort}
  A \emph{$k$-sort species} $F$ is a functor $F: \catname{Bij}^{k} \to \catname{Bij}$ which associates to each $k$-sort set $U$ a set $F \sbrac{U}$ of \emph{$k$-sort $F$-structures} and to each $k$-sort bijection $\sigma: U \to V$ a bijection $F \sbrac{\sigma}: F \sbrac{U} \to F \sbrac{V}$.
\end{definition}
