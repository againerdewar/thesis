...
Then orbits of $\pointed[XY]{\specname{CT}}_{k}$-structures under the \emph{combined} action of $\mathfrak{S}_{k+1}$ and $C_{k+1}$ are naturally identified with $\pointed[X]{\mathfrak{a}}_{k}$-structures, so:
\begin{theorem}
  \label{thm:axquot}
...
These actions of $\mathfrak{S}_{k+1}$ and $C_{k+1}$ do not commute, so the combined group which acts on $\pointed[XY]{\specname{CT}}_{k}$ is not their direct product.
It is straightforward to show that it is fact a semidirect product $C_{k+1} \rtimes \mathfrak{S}_{k+1}$.
However, we will not need to use the algebraic structure, so we leave aside the details of the multiplication.
TODO: THIS IS TOTALLY FALSE! DEAL WITH THIS!
...
\begin{proof}
  ...
  Let $\psi = \prod_{i = 1}^{l} \rho_{c_{i}} \pbrac{\sigma}$; then $\varsigma$ must induce an automorphism of $F$.
