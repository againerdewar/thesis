...
We will write $C_{k+1} = \abrac{\nu}$ where the action of $\nu$ is to rotate the choice of root white vertex one position forward with respect to the cyclic order on the white neighbors of the black root of an edge-rooted $k$-coding tree.
...
We note, however, that the set of elements of $C_{k+1} \rtimes \mathfrak{S}_{k+1}$ is simply the Cartesian product, so every element $\gamma$ has an expression $\gamma = \sigma \nu^{i}$ which acts by cyclic rotation of the white root by $i$ positions followed by application of our standard action of the permutation $\sigma$ on the resulting structure.
