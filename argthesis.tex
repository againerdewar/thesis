...
The class $\mathfrak{a}_{k}$ of $k$-trees (for $k \in \ringname{N}$) may be defined recursively:
\begin{definition}
  \label{def:ktree}
...
However, we note that, in the sense of Definition \ref{def:specdif}, since the species $\pointed[\overline{XY}]{\mathfrak{a}}_{k}$ embeds in the species $\pointed[X]{\mathfrak{a}}_{k} + \pointed[Y]{\mathfrak{a}}_{k}$ by the centering map in the proof of Theorem \ref{thm:dissymk}.
...
Finally, a $\pointed[\overline{XY}]{\specname{CT}}_{k}$-structure is simply an $X \cdot \specname{L}_{k} \pbrac{\pointed[Y]{\specname{CT}}_{k}}$-structure as described above (corresponding to the black vertex) together with a $\pointed[Y]{\specname{CT}}_{k}$-structure (corresponding to the white vertex).
For reasons that will become clear later, we note that we can incorporate the root white vertex into the linear order by making it last, thus representing a $\pointed[\overline{XY}]{\specname{CT}}_{k}$-structure instead as an $X \cdot \specname{L}_{k+1} \pbrac{\pointed[Y]{\specname{CT}}_{k}}$-structure.
...
\begin{figure}[htb]
  ...
  \end{tikzpicture}
  \caption[An example $\overline{XY}$-rooted $k$-coding tree]{An example $\pointed[\overline{XY}]{\specname{CT}}_{\kval}$-structure, rooted at the $X$-vertex and the thick edge adjoining it.}
  \label{fig:ctxyconst}
\end{figure}
...
In light of Observation \ref{obs:funcdecompct}, we now wish to adapt these ideas into explicit $\symgp{k}$- and $\symgp{k+1}$-actions on $\pointed[X]{\specname{CT}}_{k}$, $\pointed[Y]{\specname{CT}}_{k}$, and $\pointed[\overline{XY}]{\specname{CT}}_{k}$ whose orbits correspond to the various coherent orientations of single underlying rooted $k$-trees.
...
Since these actions are label-independent, we may now treat $\pointed[Y]{\specname{CT}}_{k}$ and $\pointed[\overline{XY}]{\specname{CT}}_{k}$ as $\symgp{k}$-species and $\pointed[\overline{XY}]{\specname{CT}}_{k}$ as an $\symgp{k+1}$-species.
The $\symgp{k}$- and $\symgp{k+1}$-actions on $\pointed[\overline{XY}]{\specname{CT}}_{k}$ are compatible, but we will make explicit reference to $\pointed[\overline{XY}]{\specname{CT}}_{k}$ as a $\symgp{k}$- or $\symgp{k+1}$-species whenever it is important and not completely clear from context which we mean.
...
\begin{theorem}
  \label{thm:arootquot}
  For the various rooted forms of the ordinary species $\mathfrak{a}_{k}$ as in Theorem \ref{thm:dissymk} and the various rooted $\Gamma$-species forms of $\specname{CT}_{k}$ as in Observation \ref{obs:funcdecompct} as $\symgp{k}$- and $\symgp{k+1}$-species, we have
  \begin{subequations}
    \label{eq:arootquot}
    \begin{align}
...
\begin{theorem}
  ...
  \begin{subequations}
    \label{eq:aquotci}
    \begin{align}
      \ci{\pointed[Y]{\mathfrak{a}}_{k}} &= \qgci{\symgp{k}}{\pointed[Y]{\specname{CT}_{k}}} = \frac{1}{k!} \sum_{\sigma \in \symgp{k}} \gcielt{\symgp{k}}{\pointed[Y]{\specname{CT}}_{k}}{\sigma} \label{eq:ayquotci} \\
      \ci{\pointed[\overline{XY}]{\mathfrak{a}}_{k}} &= \qgci{\symgp{k}}{\pointed[\overline{XY}]{\specname{CT}_{k}}} = \frac{1}{k!} \sum_{\sigma \in \symgp{k}} \gcielt{\symgp{k}}{\pointed[\overline{XY}]{\specname{CT}}_{k}}{\sigma} \label{eq:axyquotci} \\
      \ci{\pointed[X]{\mathfrak{a}}_{k}} &= \qgci{\symgp{k+1}}{\pointed[\overline{XY}]{\specname{CT}_{k}}} = \frac{1}{\pbrac{k+1}!} \sum_{\sigma \in \symgp{k+1}} \gcielt{\symgp{k+1}}{\pointed[\overline{XY}]{\specname{CT}}_{k}}{\sigma} \label{eq:axquotci}
    \end{align}
  \end{subequations}
\end{theorem}
...
In Observation \ref{obs:funcdecompct}, the functional equations for the ordinary species $\pointed[Y]{\specname{CT}}_{k}$ and $\pointed[\overline{XY}]{\specname{CT}}_{k}$ both include terms of the form $\specname{L}_{k} \circ \pointed[Y]{\specname{CT}}_{k}$.
...
The permutation $\sigma$ acts trivially on $Y$ and $\specname{E}$ and acts on each of the component $X \cdot \specname{L}_{k} \pbrac{\pointed[Y]{\specname{CT}}_{k}}$-structures independently.
For each of these component structures, by Lemma \ref{lem:ctyinvar}, we have that $\sigma$ is an automorphism if and only if $\Delta \sigma$ carries each $\pointed[Y]{\specname{CT}}_{k}$-structure to its $\Delta^{-1} \pbrac{\rho_{i} \pbrac{\Delta \sigma}}$-image.
...
\begin{theorem}
  ...
  \begin{multline}
    \label{eq:ctyfuncci}
    \gcielt{\symgp{k}}{\pointed[Y]{\specname{CT}}_{k}}{\sigma} = p_{1} \sbrac{y} \\
    \times Z_{\specname{E}} \circ \pbrac{p_{1} \sbrac{x} \cdot \prod_{c \in C \pbrac{\sigma}} \gcieltvars{\symgp{k}}{\pointed[Y]{\specname{CT}}_{k}}{\Delta^{-1} \prod_{i \in c} \rho_{i} \pbrac{\Delta \sigma}}{p_{\abs{c}} \sbrac{x}, p_{2 \abs{c}} \sbrac{x}, \dots; p_{\abs{c}} \sbrac{y}, p_{2 \abs{c}} \sbrac{y}, \dots}}.
  \end{multline}
...
\begin{theorem}
  ...
  \begin{multline}
    \label{eq:ctxyfuncci}
    \gcielt{\symgp{k+1}}{\pointed[\overline{XY}]{\specname{CT}}_{k}}{\sigma} = p_{1} \sbrac{x} \\
    \times \prod_{c \in C \pbrac{\sigma}} \gcieltvars{\symgp{k}}{\pointed[Y]{\specname{CT}}_{k}}{\prod_{i \in c} \rho_{i} \sbrac{\sigma}}{p_{\abs{c}} \sbrac{x}, p_{2 \abs{c}} \sbrac{x}, \dots, p_{\abs{c}} \sbrac{y}, p_{2 \abs{c}} \sbrac{y}, \dots}.
  \end{multline}
...
\begin{theorem}[Cycle index for the species of $k$-trees]
  ...
  \begin{subequations}
    \label{eq:akci}
    \begin{align}
      \ci{\mathfrak{a}_{k}} &= \frac{1}{\pbrac{k+1}!} \sum_{\sigma \in \symgp{k+1}} \gcielt{\symgp{k+1}}{\pointed[\overline{XY}]{\specname{CT}}_{k}}{\sigma} + \frac{1}{k!} \sum_{\sigma \in \symgp{k}} \gcielt{\symgp{k}}{\pointed[Y]{\specname{CT}}_{k}}{\sigma} - \frac{1}{k!} \sum_{\sigma \in \symgp{k}} \gcielt{\symgp{k}}{\pointed[\overline{XY}]{\specname{CT}}_{k}}{\sigma} \label{eq:akciexplicit} \\
...
Equation \eqref{eq:akci} in Theorem \ref{thm:akci} gives a recursive formula for the cycle index of the ordinary species $\mathfrak{a}_{k}$ of $k$-trees.
...
\begin{theorem}
  \label{thm:ctogf}
  For $\pointed[Y]{\specname{CT}}_{k}$ the $\symgp{k}$-species of $Y$-rooted $k$-coding trees and $\pointed[\overline{XY}]{\specname{CT}}_{k}$ the $\symgp{k+1}$-species of edge-rooted $k$-coding trees, the corresponding $\Gamma$-ordinary generating functions are given by
  \begin{align}
    \widetilde{\pointed[Y]{\specname{CT}}_{k}} \pbrac{\sigma} \pbrac{x} &= \exp \pbrac{ \sum_{n \geq 1} \frac{x^{n}}{n} \cdot \prod_{c \in C \pbrac{\sigma^{n}}} \widetilde{\pointed[Y]{\specname{CT}}_{k}} \pbrac{\Delta^{-1} \prod_{i \in c} \rho_{i} \pbrac{\Delta \sigma^{n}}} \pbrac{x^{\abs{c}}}} \label{eq:ctyogf} \\
  ...
  \end{align}
  where $\widetilde{\pointed[Y]{\specname{CT}}_{k}}$ is a $\symgp{k}$-generating function and $\widetilde{\pointed[\overline{XY}]{\specname{CT}}_{k}}$ is a $\symgp{k+1}$-generating function.
\end{theorem}
...
\begin{subequations}
  \label{eq:rest2trees}
  \begin{align}
    \gci{\symgp{2}}{\pointed[Y]{\specname{CT}}_{2}} &= p_{1} \sbrac{y} \cdot Z_{\specname{E}} \circ \pbrac{p_{1} \sbrac{x} \cdot \prod_{c \in C \pbrac{\sigma}} \gcieltvars{\symgp{2}}{\pointed[Y]{\specname{CT}}_{2}}{e}{p_{\abs{c}} \sbrac{x}, p_{2 \abs{c}} \sbrac{x}, \dots; p_{\abs{c}} \sbrac{y}, p_{2 \abs{c}} \sbrac{y}, \dots}} \label{eq:ctyfuncci2} \\
    \gci{\symgp{3}}{\pointed[\overline{XY}]{\specname{CT}}_{2}} &= p_{1} \sbrac{x} \cdot \prod_{c \in C \pbrac{\sigma}} \gcieltvars{\symgp{2}}{\pointed[Y]{\specname{CT}}_{2}}{\rho \pbrac{\sigma}^{\abs{c}}}{p_{\abs{c}} \sbrac{x}, p_{2 \abs{c}} \sbrac{x}, \dots; p_{\abs{c}} \sbrac{y}, p_{2 \abs{c}} \sbrac{y}, \dots}. \label{eq:ctxyfuncci2}
  \end{align}
\end{subequations}
