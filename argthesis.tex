\documentclass[distribution,boldmathhdr,draft]{brandiss} % Compactify for drafting (do *not* use these options for final submission!)
% \documentclass[distribution,boldmathhdr]{brandiss} % Pretty version for distribution (*not* for GSAS submission!)
% \documentclass[online,submission,boldmathhdr]{brandiss} % Online version (states that signature page is on file)
% \documentclass[submission,boldmathhdr]{brandiss} % Print version
...
We note here also that the definitions of species embedding and species covering are not strictly categorical; they are \emph{not} in general the epi- and monomorphisms in the category of species.
Explicitly, if $\phi$ is a species map, the diagram
\begin{equation*}
  \begin{tikzpicture}[every node/.style={fill=white}]
    \matrix (m) [matrix of math nodes, row sep=2em, column sep=3em]
      ...
      \path[->,font=\scriptsize]
      (m-1-1) edge node {$F \sbrac{\sigma}$} (m-1-4)
      edge node {$\phi_{A}$} (m-3-1)
      (m-1-4) edge node {$\phi_{B}$} (m-3-4)
      (m-2-2) edge node {$\sigma$} (m-2-3)
      edge node {$F$} (m-1-1)
      edge node {$G$} (m-3-1)
      (m-2-3) edge node {$F$} (m-1-4)
      edge node {$G$} (m-3-4)
      (m-3-1) edge node {$G \sbrac{\sigma}$} (m-3-4);
  \end{tikzpicture}
\end{equation*}
does \emph{not} commute unless $\phi$ is a species isomorphism.
...
However, we note that, in the sense of Definition \ref{def:specsub}, since the species $\pointed[\overline{XY}]{\mathfrak{a}}_{k}$ embeds in the species $\pointed[X]{\mathfrak{a}}_{k} + \pointed[Y]{\mathfrak{a}}_{k}$ by the centering map in the proof of Theorem \ref{thm:dissymk}.
