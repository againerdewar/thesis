...
\begin{definition}
  ...
  Explicitly, for $F$ a $\Gamma$-species, the following diagram commutes:
  \begin{equation*}
    \begin{tikzpicture}[every node/.style={auto}]
      \matrix (m) [matrix of math nodes, row sep=4em, column sep=4em]
      {
...
As an aside, we also note that the fact that the obstruction of Proposition \ref{prop:notransac} occurs only for $k \geq 3$ would suggest that a more straightforward course than ours would be available specifically in the case of $2$-trees.
