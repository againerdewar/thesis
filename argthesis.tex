...
\begin{theorem}\label{thm:bctreeenc}
  $\beta$ induces an isomorphism of species $\specname{CT}_{k} \simeq \specname{CT}_{k}$.
\end{theorem}
...
\begin{figure}[htbp]
  ...
  \caption[The permutation-modifying map $\rho$]{Application of a permutation $\sigma$ to an $H_{k}$-structure rooted at the black vertex $A$.
    The vertices $2$ and $3$ are mirror in the original orientation (lower set of edges), so we must preserve this mirror relation when we apply $\sigma$.
...
Moreover, we can easily pass this action to the species $\pointed[Y]{\mathfrak{a}}_{O, k}$ of coherently-oriented $k$-trees rooted at fronts, and its orbits are clearly the underlying unoriented front-rooted $k$-trees of $\pointed[Y]{\mathfrak{a}}_{k}$.
...
Each $\pointed[X]{\mathfrak{a}}_{k}$-structure (that is, hedron-rooted $k$-tree) $T$ is naturally identified with the set of $\pbrac{\pointed[X]{\mathfrak{a}}_{O, k} = \pointed[X]{\specname{CT}}_{k}}$-structures that are the coherent orientations of $T$.
...
\begin{theorem}
  ...
  \end{multline}
  where $C \pbrac{\sigma}$ denotes the set of cycles of $\sigma$ and where the inner product is taken over the elements $i$ of the cycle $c$ in an order determined by fixing a basepoint (arbitrarily) and linearizing.
\end{theorem}
...
\begin{theorem}
  ...
  \begin{multline}
    ...
    \cdot \prod_{c \in C \pbrac{\sigma \zeta^{j}}} Z_{F_{k}} \pbrac{\prod_{i \in c} \rho_{\zeta^{j} \pbrac{i}} \sbrac{\sigma}} \sbrac{p_{\abs{c}} \sbrac{x}, p_{2 \abs{c}} \sbrac{x}, \dots, p_{\abs{c}} \sbrac{y}, p_{2 \abs{c}} \sbrac{y}, \dots}.
  \end{multline}
...
\begin{theorem}[Cycle index for the species of $k$-trees]
  ...
  \label{thm:ktreecyc}
  \begin{multline}
    Z_{\mathfrak{a}_{k}} = \frac{1}{k! \pbrac{k+1}} \sum_{\substack{\sigma \in \mathfrak{S}_{k} \\ i \in \cbrac{0, 1, \dots, k}}} Z_{\pointed[XY]{\specname{CT}}_{k}} \pbrac{\sigma, \zeta^{i}} \\
    + \frac{1}{k!} \sum_{\sigma \in \mathfrak{S}_{k}} Z_{\pointed[Y]{\specname{CT}}_{k}} \pbrac{\sigma} - \frac{1}{k!} \sum_{\sigma \in \mathfrak{S}_{k}} \pbrac{Z_{F_{k}} \pbrac{\sigma} \cdot Z_{H_{k}} \pbrac{\sigma}}.
  \end{multline}
\end{theorem}
