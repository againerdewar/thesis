...
\begin{proof}
  ...
  Now, by way of induction, suppose $\specname{CT}_{k}$ and $\specname{CT}_{k}$ have contact of order $n \geq 1$; that is, that the encoding $\beta$ is a natural bijection for up to $n$ hedra and up to $n$ black vertices.
...
\begin{figure}[htbp]
  ...
  \caption[The permutation-modifying map $\rho$]{Application of a permutation $\sigma$ to an $H_{k}$-structure rooted at the black vertex $A$.
    ...
    $\sigma$ moves $3$ from the $\pbrac{i+a}$th place to the $\sigma \pbrac{i+a}$th, so $\rho_{i} \pbrac{\sigma}$ must carry $2$ from the $a$th place to the $\pbrac{\sigma \pbrac{i+a} - \sigma \pbrac{i}}$th.}
  \label{fig:rhoapp}
\end{figure}
