...
We now define a map $\beta: \mathfrak{a}_{O, k} \sbrac{n} \to \specname{CT}_{k} \sbrac{n}$.
...
\begin{theorem}\label{thm:bctreeenc}
  $\beta$ induces an isomorphism of species $\mathfrak{a}_{O, k} \simeq \specname{CT}_{k}$.
\end{theorem}
...
$\mathfrak{a}_{O, k}$ and $\specname{CT}_{k}$ are isomorphic as species, but $k$-coding trees are much simpler than coherently-oriented $k$-trees as graphs.
...
We could use Lagrange inversion or another method to compute explicit cycle indices for all the species described so far in this section, culminating in complete enumerative data for $\specname{CT}_{k} = \mathfrak{a}_{O, k}$.
