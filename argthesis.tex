...
\begin{proof}
  ...
  Define $\fix_{\sigma} \sbrac{n}$ to be the number of $H_{k}$-structures with $n$ black vertices for which $\sigma$ induces an automorphism and let $p_{\sigma}$ denote $p_{\gamma}$ for $\gamma$ the permutation on labels which is specified by $\sigma$.
  We then wish to calculate $\fix_{\sigma} \sbrac{n} p_{\sigma}$ for each $\sigma \in \mathfrak{S}_{k}$.
  ...
  Therefore, with respect to the cycle $c$, we have $\fix_{\psi}$ choices for the structure $F$, and the label permutation $\gamma$ which is induced by this isomorphism has an $l$-cycle for every $1$-cycle of $\psi$, a $2l$-cycle for every $2$-cycle of $\psi$, and so on.
  The contribution to the cycle index is then $\fix_{\psi} p_{\psi}^{l}$, which is exactly the term  \begin{equation*}
