...
\begin{definition}
  \label{def:specmap}
  Let $F$ and $G$ be species. A \emph{species map} $\phi$ is a natural transformation $\phi: F \to G$ --- that is, an association of each set $A \in \catname{FinBij}$ to a set map $\phi_{A} \in \catname{FinSet}$ such that the following diagram commutes:
  \begin{equation*}
