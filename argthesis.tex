...
We note also that species addition is associative and commutative (up to species isomorphism), and furthermore the empty species $\mathbf{0}: A \mapsto \varnothing$ is an additive identity, so species with addition form an abelian monoid.
...
However, the formula in Theorem \ref{thm:speccompci} fails to be well-defined for any $\ci{G}$ with non-zero constant term (corresponding to species $G$ with nonempty $G \sbrac{\varnothing}$) unless $\ci{Z}$ has finite degree (corresponding to species $F$ with support only in finitely many degrees).
Consider the following example:
\begin{example}
  Let $\specname{E}$ denote the species of sets, $\specname{E}_{3}$ its restriction to sets with three elements, $\mathbf{1}$ the species described above (which has one empty structure), and $X$ the species of singletons (which has one order-$1$ structure).
  If $\specname{E} \pbrac{\mathbf{1} + X}$ were well-defined, it would denote the species of `partially-labeled sets'.
  However, for fixed cardinality $n$, there is an $\specname{E} \pbrac{\mathbf{1} + X}$-structure on $n$ labels \emph{for each nonnegative $k$}---specifically, the set $\sbrac{n}$ together with $k$ unlabeled elements.
  ...
  However, the situation for $\specname{E}_{3} \pbrac{\mathbf{1} + X}$ is entirely different.
  ...
  Moreover, by discarding the unlabeled elements, we can clearly see that $\specname{E}_{3} \pbrac{\mathbf{1} + X} = \sum_{i = 0}^{3} \specname{E}_{i}$.
\end{example}
...
\begin{theorem}
  ...
  \begin{multline}
    ...
    \times Z_{\specname{E}} \circ \pbrac{p_{1} \sbrac{x} \cdot \prod_{c \in C \pbrac{\sigma}} \gci{\symgp{k}}{\pointed[Y]{\specname{CT}}_{k}} \pbrac{\Delta^{-1} \prod_{i \in c} \rho_{i} \pbrac{\Delta \sigma}} \sbrac{p_{\abs{c}} \sbrac{x}, p_{2 \abs{c}} \sbrac{x}, \dots; p_{\abs{c}} \sbrac{y}, p_{2 \abs{c}} \sbrac{y}, \dots}}.
  \end{multline}
