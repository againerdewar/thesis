...
Using the computer algebra system Sage 1.7.4 \cite{sage}, we have implemented some algorithms to do exactly this.
...
\lstinputlisting[caption=Sage code to compute numbers of bipartite blocks, label=lst:ktcode, language=Python]{python/bpblocks.py}
...
For example, the first thirty terms of the generating function for $8$-trees in Table \ref{tab:8trees} were computed on a modern desktop-class computer in approximately one minute.

\lstinputlisting[caption=Sage code to compute numbers of $k$-trees, label=lst:ktcode, language=Python]{python/ktrees.py}
