...
Finally, consider instead a $k$-coding tree rooted at an edge (that is, at an adjacent black-white pair) from the class $\pointed[\overline{XY}]{\specname{CT}}_{k}$.
...
\begin{theorem}\label{thm:funcdecompct}
  For the species $H_{k}$, $F_{k}$, $\pointed[X]{\specname{CT}}_{k}$, $\pointed[Y]{\specname{CT}}_{k}$, and $\pointed[\overline{XY}]{\specname{CT}}_{k}$ as above, we have the functional relations
  \begin{subequations}
    \label{eq:ctfunc}
    \begin{align}
      ...
      \pointed[\overline{XY}]{\specname{CT}}_{k} &= F_{k} \cdot
...
\subsection{Quotients for $\pointed[Y]{\mathfrak{a}}_{k}$ and $\pointed[\overline{XY}]{\mathfrak{a}}_{k}$}\label{ss:ayact}
...
Similarly, we may extend this $\symgp{k}$-action to the species $\pointed[\overline{XY}]{\specname{CT}}_{k}$ of edge-rooted $k$-coding trees.
By equation \eqref{eq:ctxyfunc}, we can decompose a $\pointed[\overline{XY}]{\specname{CT}}_{k}$-structure into an $F_{k}$-structure and an $H_{k}$-structure, each of which has a well-defined $\symgp{k}$-action, which we simply pass back up to $\pointed[\overline{XY}]{\specname{CT}}_{k}$.
By a similar argument to that above, it is clear that orbits of $\pointed[\overline{XY}]{\specname{CT}}_{k}$-structures are in natural bijection with $\pointed[\overline{XY}]{\mathfrak{a}}_{k}$-structures, so:
\begin{theorem}
  \label{thm:axyquot}
  For $\pointed[\overline{XY}]{\mathfrak{a}}_{k}$ the species of $k$-trees rooted at hedra with designated fronts, $\pointed[\overline{XY}]{\specname{CT}}_{k}$ the species of $k$-coding trees rooted at edges, and $H_{k}$ and $F_{k}$ the component species of black- and white-vertex-rooted $k$-coding tree components, we have
  \begin{equation}
    \label{eq:axyquot}
    \pointed[\overline{XY}]{\mathfrak{a}}_{k} = \faktor{\pointed[\overline{XY}]{\specname{CT}}_{k}}{\symgp{k}}  = \pbrac{\faktor{H_{k}}{\symgp{k}}} \cdot \pbrac{\faktor{F_{k}}{\symgp{k}}}.
  \end{equation}
  for the $\symgp{k}$-action described above.
\end{theorem}
...
We have thus far defined $\symgp{k}$-actions on the species $H_{k}$, $F_{k}$, $\pointed[Y]{\specname{CT}}_{k}$, and $\pointed[\overline{XY}]{\specname{CT}}_{k}$, and used these to characterize the species $\pointed[Y]{\mathfrak{a}}_{k}$ and $\pointed[\overline{XY}]{\mathfrak{a}}_{k}$.
...
If, however, we identify each $\pointed[X]{\specname{CT}}_{k}$-structure $T$ with the set of $\pointed[\overline{XY}]{\specname{CT}}_{k}$-structures which are obtained by rooting at all the edges incident to the root black vertex of $T$, we have an $\symgp{k}$-action on the latter set whose orbits are $\pointed[\overline{XY}]{\mathfrak{a}}_{k}$-structures.
Moreover, if we write $C_{k+1}$ for the cyclic group with $k+1$ elements, there is a natural $C_{k+1}$-action on $\pointed[\overline{XY}]{\specname{CT}}_{k}$-structures which rotates the choice of root white vertex around the root black vertex with respect to the cyclic order on those white neighbors.
...
These actions of $\symgp{k}$ and $C_{k+1}$ do not commute, so the combined group which acts on $\pointed[\overline{XY}]{\specname{CT}}_{k}$ is not their direct product.
...
If we treat the $\specname{C}_{k+1}$-structure on the white neighbors of the black root of a $\pointed[\overline{XY}]{\specname{CT}}_{k}$-structure as an $\specname{L}_{k+1}$-structure (rather than the usual $\specname{L}_{k}$) by appending the white root to the \emph{end} of the linear order, there is a natural $\symgp{k+1}$-action on this linear order by permutations.
...
Once some pair $\pbrac{\zeta^{i}, \sigma}$ is allowed to act on the $\specname{L}_{k+1}$-structure at the root of a $\pointed[\overline{XY}]{\specname{CT}}_{k}$-structure, however, we must adjust the orderings of the white children of non-root black vertices to preserve the coherence of the orientation of the associated $k$-tree.
...
With the interaction of the actions of $C_{k+1}$ and $\symgp{k}$ on $\pointed[\overline{XY}]{\specname{CT}}_{k}$-structures so defined, it is clear that each orbit of the combined action $C_{k+1} \rtimes \symgp{k}$ corresponds to the set of all the ways to take a given hedron-rooted $k$-tree, select a front of the root hedron, and coherently orient the result.
Thus:
\begin{theorem}
  \label{thm:axquot}
  For $\pointed[X]{\mathfrak{a}}_{k}$ the species of $k$-trees rooted at hedra and $\pointed[\overline{XY}]{\specname{CT}}_{k}$ the species of $k$-coding trees rooted at edges, we have
  \begin{equation}
    \label{eq:axquot}
    \pointed[X]{\mathfrak{a}}_{k} = \faktor{\pointed[\overline{XY}]{\specname{CT}}_{k}}{\pbrac{C_{k+1} \rtimes \symgp{k}}}
  \end{equation}
  for the action of $C_{k+1} \rtimes \symgp{k}$ described above.
\end{theorem}

We now have all the pieces of the original dissymmetry Theorem \ref{thm:dissymk} save $\mathfrak{a}_{k}$ itself characterized in terms of quotients of species of rooted $k$-coding trees: $\pointed[Y]{\mathfrak{a}}_{k}$ in \ref{thm:ayquot}, $\pointed[\overline{XY}]{\mathfrak{a}}_{k}$ in \ref{thm:axyquot}, and $\pointed[X]{\mathfrak{a}}_{k}$ in \ref{thm:axquot}.
...
\begin{theorem}
  ...
  \begin{equation}
    \label{eq:dissymkci}
    \gci{\symgp{k}}{\mathfrak{a}_{k}} = \gci{\symgp{k}}{\pointed[X]{\mathfrak{a}}_{k}} + \gci{\symgp{k}}{\pointed[Y]{\mathfrak{a}}_{k}} - \gci{\symgp{k}}{\pointed[\overline{XY}]{\mathfrak{a}}_{k}}.
  \end{equation}
\end{theorem}
...
\begin{theorem}
  ...
  \begin{subequations}
    ...
    \begin{align}
      ...
      \ci{\pointed[\overline{XY}]{\mathfrak{a}}_{k}} &= \qgci{\symgp{k}}{\pointed[\overline{XY}]{\specname{CT}_{k}}} = \frac{1}{k!} \sum_{\sigma \in \symgp{k}} \gci{\symgp{k}}{\pointed[\overline{XY}]{\specname{CT}}_{k}} \pbrac{\sigma} = \frac{1}{k!} \sum_{\sigma \in \symgp{k}} \pbrac{\gci{\symgp{k}}{F_{k}} \pbrac{\sigma} \cdot \gci{\symgp{k}}{H_{k}} \pbrac{\sigma}} \label{eq:axyquotci} \\
      \ci{\pointed[X]{\mathfrak{a}}_{k}} &= \qgci{\symgp{k} \rtimes C_{k+1}}{\pointed[\overline{XY}]{\specname{CT}_{k}}} = \frac{1}{k! \pbrac{k+1}} \sum_{\substack{\sigma \in \symgp{k} \\ i \in \cbrac{0, 1, \dots, k}}} \gci{\symgp{k}}{\pointed[\overline{XY}]{\specname{CT}}_{k}} \pbrac{\sigma, \zeta^{i}} \label{eq:axquotci}
    \end{align}
  \end{subequations}
\end{theorem}
...
\subsection{Rooted components: $\pointed[Y]{\specname{CT}}_{k}$ and $\pointed[\overline{XY}]{\specname{CT}}_{k}$}\label{ss:ktcycindroot}
We now consider the $\symgp{k}$-species $\pointed[Y]{\specname{CT}}_{k}$ of $k$-coding trees rooted at white vertices and the $\pbrac{C_{k+1} \rtimes \symgp{k}}$-species $\pointed[\overline{XY}]{\specname{CT}}_{k}$ of $k$-coding trees rooted at edges.
...
\begin{theorem}
  ...
  The $\pbrac{\symgp{k} \times C_{k+1}}$-cycle index for the species $\pointed[\overline{XY}]{\specname{CT}}_{k}$ is given by
  \begin{multline}
    \label{eq:ctxyfuncci}
    \gci{\symgp{k} \rtimes C_{k+1}}{\pointed[\overline{XY}]{\specname{CT}}_{k}} \pbrac{\sigma, \zeta^{j}} = p_{1} \sbrac{x} \\
...
\begin{theorem}[Cycle index for the species of $k$-trees]
  For $\gci{\symgp{k}}{\pointed[Y]{\specname{CT}}_{k}}$ as in equation \eqref{eq:ctyfuncci}, $\gci{\symgp{k} \rtimes C_{k+1}}{\pointed[\overline{XY}]{\specname{CT}}_{k}}$ as in equation \eqref{eq:ctxyfuncci}, $\gci{\symgp{k}}{H_{k}}$ as in equation \eqref{eq:hfuncci}, and $\gci{\symgp{k}}{F_{k}}$ as in equation \eqref{eq:ffuncci}, we have:
  \label{thm:ktreecyc}
  \begin{multline}
    \quad \ci{\mathfrak{a}_{k}} = \frac{1}{k! \pbrac{k+1}} \sum_{\substack{\sigma \in \symgp{k} \\ i \in \cbrac{0, 1, \dots, k}}} \gci{\symgp{k} \rtimes C_{k+1}}{\pointed[\overline{XY}]{\specname{CT}}_{k}} \pbrac{\sigma, \zeta^{i}} \\
