...
\begin{theorem}\label{thm:cycindlab}
  ...
  \begin{equation}\label{eq:cycindlab}
    F \pbrac{x} = \ci{f} \pbrac{x, 0, 0, \dots}.
  \end{equation}
\end{theorem}
\begin{theorem}\label{thm:cycindunlab}
  ...
  \begin{equation}\label{eq:cycindunlab}
    \tilde{F} \pbrac{x} = \ci{F} \pbrac{x, x^{2}, x^{3}, \dots}.
  \end{equation}
\end{theorem}
...
\begin{theorem}
  ...
  \begin{equation}
    ...
    \ci{F'} \pbrac{p_{1}, p_{2}, \dots} = \pbrac{\frac{\partial}{\partial p_{1}} \ci{F}} \pbrac{p_{1}, p_{2}, \dots}.
  \end{equation}
\end{theorem}
We note that we cannot in general recover $\ci{F}$ from $\ci{F'}$, since there may be terms in $\ci{F}$ which have no $p_{1}$-component (corresponding to $F$-structures which have no automorphisms with fixed points).
...
\begin{theorem}
  ...
  \begin{equation}
    ...
    \ci{\pointed{F}} = \ci{X} \cdot \ci{F'}.
  \end{equation}
\end{theorem}
Note that, again, we cannot in general recover $\ci{F}$ from $\ci{\pointed{F}}$, for the same reasons as in the case of $\ci{F'}$.

\section{Multisort species}\label{s:mult}
A species $F$ as defined in \ref{def:species} is a functor $F: \mathbf{B} \to \mathbf{S}$; an $F$-structure in $F \sbrac{A}$ takes its labels from the set $A$.
...
\begin{multline}
  ...
  \ci{F} \pbrac{p_{1} \sbrac{1}, p_{2} \sbrac{1}, \dots; p_{1} \sbrac{2}, p_{2} \sbrac{2}, \dots; \dots; p_{1} \sbrac{k}, p_{2} \sbrac{k}, \dots} = \\
...
\begin{theorem}\label{thm:qsci}
  ...
  \begin{equation}
    \label{eq:quotcycind}
    \ci{F / \Gamma} = \frac{1}{\abs{\Gamma}} \sum_{\gamma \in \Gamma} \gci{\Gamma}{F} \pbrac{\gamma} = \frac{1}{\abs{\Gamma}} \sum_{\substack{n \geq 0 \\ \sigma \in \symgp{n} \\ \gamma \in \Gamma}} \frac{1}{n!} \pbrac{\gamma \cdot F \sbrac{\sigma}} p_{\sigma}.
  \end{equation}
\end{theorem}
...
\begin{theorem}
  \begin{equation}
    \label{eq:tcibc}
    \gci{\symgp{2}}{\specname{BC}} \pbrac{\tau} = \sum_{\substack{n \geq 0 \\ \text{$n$ even}}} \sum_{\lambda \vdash \frac{n}{2}} 2^{l \pbrac{\lambda}} \frac{p_{2 \lambda}}{z_{2 \lambda}} \prod_{i} 2^{\ceil{\frac{\lambda_{i}}{2}}} \prod_{i < j} 2^{\gcd \pbrac{\lambda_{i}, \lambda_{j}}}
  \end{equation}
\end{theorem}
...
\begin{theorem}
  \begin{equation}
    \ci{\specname{CBP}} = \frac{1}{2} \pbrac{\gci{\symgp{2}}{\specname{CBC}} \pbrac{e} + \gci{\symgp{2}}{\specname{CBC}} \pbrac{\tau}}.
  \end{equation}
\end{theorem}
...
\subsection{The dissymmetry theorem for $k$-trees}\label{ss:dissymk}
...
\begin{theorem}
  \label{thm:dissymk}
  The species $\pointed[X]{\mathfrak{a}}_{k}$ and $\pointed[Y]{\mathfrak{a}}_{k}$ of $k$-trees rooted at hedra and fronts respectively, $\pointed[XY]{\mathfrak{a}}_{k}$ of $k$-trees rooted at a hedron with a designated front, and $\mathfrak{a}_{k}$ of unrooted $k$-trees are related by the equation
  \begin{equation}
    \label{eq:dissymk}
    \pointed[X]{\mathfrak{a}}_{k} + \pointed[Y]{\mathfrak{a}}_{k} = \mathfrak{a}_{k} + \pointed[XY]{\mathfrak{a}}_{k}
  \end{equation}
  as an isomorphism of species.
\end{theorem}

\begin{proof}
  We give a bijective, natural map from $\pbrac{\pointed[X]{\mathfrak{a}}_{k} + \pointed[Y]{\mathfrak{a}}_{k}}$-structures on the left side to $\pbrac{\mathfrak{a}_{k} + \pointed[XY]{\mathfrak{a}}_{k}}$-structures on the right side.
  ...
  This map is also a natural bijection, in this case from the set of $k$-trees rooted at vertices which are \emph{not} their centers to the set $\pointed[XY]{\mathfrak{a}}_{k}$ of $k$-trees rooted at an adjacent hedron-front pair.
...
\begin{corollary}
  ...
  \begin{equation}
    ...
    \mathfrak{a}_{k} = \pointed[X]{\mathfrak{a}}_{k} + \pointed[Y]{\mathfrak{a}}_{k} - \pointed[XY]{\mathfrak{a}}_{k}.
  \end{equation}
\end{corollary}
...
TODO: Make sure that $\vec{\mathfrak{a}}_{k}$ is even defined somewhere.
...
Finally, consider instead a $k$-coding tree rooted at an edge (that is, at an adjacent black-white pair) from the class $\pointed[XY]{\specname{CT}}_{k}$.
Deleting the root edge (leaving the vertices) breaks the tree into two pieces, one of which is an $H_{k}$-structure and the other of which is an $F_{k}$ structure.
Moreover, the original $k$-coding tree can be reconstructed from such pieces by simply attaching the root black and white vertices by an edge.

Then:
\begin{theorem}\label{thm:funcdecompct}
  For the species $H_{k}$, $F_{k}$, $\pointed[X]{\specname{CT}}_{k}$, $\pointed[Y]{\specname{CT}}_{k}$, and $\pointed[XY]{\specname{CT}}_{k}$ as above, we have the functional relations
  \begin{subequations}
    \label{eq:ctfunc}
    \begin{align}
      H_{k} &= X \cdot \specname{C}'_{k+1} \pbrac{F_{k}} = X \cdot \specname{L}_{k+1} \pbrac{F_{k}} \label{eq:hfunc} \\
      F_{k} &= Y \cdot \specname{E}' \pbrac{H_{k}} = Y \cdot \specname{E} \pbrac{H_{k}} \label{eq:ffunc} \\
      \pointed[X]{\specname{CT}}_{k} &= X \cdot \specname{C}_{k+1} \pbrac{F_{k}} \label{eq:ctxfunc} \\
      \pointed[Y]{\specname{CT}}_{k} &= Y \cdot \specname{E} \pbrac{H_{k}} \label{eq:ctyfunc} \\
      \pointed[XY]{\specname{CT}}_{k} &= F_{k} \cdot
      H_{k}. \label{eq:ctxyfunc}
    \end{align}
  \end{subequations}
...
It remains, then, to apply the theory of quotient species to pass back from $k$-coding trees to generic $k$-trees.

\section{Generic $k$-trees}\label{s:genkt}
\subsection{Quotients for $\pointed[Y]{\mathfrak{a}}_{k}$ and $\pointed[XY]{\mathfrak{a}}_{k}$}\label{ss:ayact}
...
Instead, we will use the additional structure on \emph{rooted} coherently-oriented $k$-trees to linearize these $\pbrac{k+1}$-cyclic orders into $k$-linear orders, for which there is a natural action of $\symgp{k}$.
...
Indeed, in \cite{gessel:spec2trees}, the fact that there \emph{is} a well-defined action of $\symgp{2}$ on $\cyc_{\sbrac{3}}$ which simply reverses the cyclic ordering is exploited to study $2$-trees species-theoretically.
...
There is a natural label-independent action of $\symgp{k}$ on the set of these linear orders under which, for example, $\sigma = \pbrac{1 2}$ switches the first and second places in an ordering.
If we simply apply some $\sigma \in \symgp{k}$ in this manner to the root of an $H_{k}$-structure, however, the resulting $H_{k}$-structure will \emph{not} correspond to a different coherent orientation of the same underlying $k$-tree; the coherence of the ordering of the white neighbors of the root black vertex with the orderings of the descendant $H_{k}$ structures will be disturbed.
...
\begin{theorem}
  \label{thm:rhodef}
  If a permutation $\sigma \in \symgp{k}$ is applied to the orientation of the root black vertex in an $H_{k}$-structure, the permutation which must be applied to a child black vertex which was attached to the $i$th white vertex (with respect to the linear ordering induced by the orientation) to preserve coherence is $\rho_{i} \pbrac{\sigma}$, where $\rho_{i}$ is the map
  \begin{equation}
    \label{eq:rhodef}
...
\end{theorem}
\begin{proof}
  Let $A$ denote the base black vertex and $B$ denote an immediate black descendant, which is attached to $A$ by the white vertex $1$, which we suppose to be in position $i$ in the linear order induced by the original orientation of $A$.
  Let $2$ denote the white child of $B$ which is $a$th in the linear order induced by the original orientation around $B$.
  It is mirror to the white child $3$ of $A$ which is $\pbrac{i+a}$th in the linear order induced by the original orientation around $A$.
  After the action of $\sigma$ is applied, vertex $3$ is $\sigma \pbrac{i+a}$th in the new linear order around $A$.
  We require that $2$ is still mirror to $3$, so we must move it to position $\sigma \pbrac{i + a} - \sigma \pbrac{i}$ when we create a new linear order around $B$.
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}
    ...
    \EA[style=fnode](A){3}
    \path (A) edge [bend right=45, thick] node [below](a3d){$i+a$} (3);
    \path (A) edge [bend left=45, thick] node [above](a3u){$\sigma \pbrac{i+a}$} (3);
    \path (a3d) edge [->, dashed, thick] node [auto]{$\sigma$} (a3u);

    \WE[style=fnode](A){1}
    \path (A) edge [bend left=45, thick] node [below](a1d){$i$} (1);
    \path (A) edge [bend right=45, thick] node [above](a1u){$\sigma \pbrac{i}$} (1);
    \path (a1d) edge [->, dashed, thick] node [auto]{$\sigma$} (a1u);
    
    \WE[style=hnode](1){B}
    \cyccwr{(B)};
    \path (B) edge [thick] node [auto](b1){$0$} (1);

    \WE[style=fnode](B){2}
    \path (B) edge [bend left=45, thick] node [below](b2d){$a$} (2);
    \path (B) edge [bend right=45, thick] node [above](b2u){$\sigma \pbrac{i+a} - \sigma \pbrac{i}$} (2);
  ...
  \end{tikzpicture}
  \caption[The permutation-modifying map $\rho$]{Application of a permutation $\sigma$ to an $H_{k}$-structure rooted at the black vertex $A$.
...
This yields a well-defined $\symgp{k}$-action on the species $H_{k}$ which, we note, is completely label-independent and thus naturally commutes with the $\pbrac{\symgp{n} \times \symgp{kn+1}}$-action on labels, as we might hope in order to apply the theory of $\Gamma$-species.
Moreover, the orbits of this $\symgp{k}$-action are exactly the sets of $H_{k}$-structures which are different coherent orientations of the same underlying $k$-tree structure.
TODO: justify this claim?

We also need to consider the associated action of $\symgp{k}$ on $F_{k}$-structures, but there is nothing new to be done.
Simply applying the $\symgp{k}$-action defined previously to each of the $H_{k}$-structures that are immediate children of the root white vertex of an $F_{k}$-structure suffices to preserve coherence.
Moreover, again, the orbits of this $\symgp{k}$-action are the sets of $F_{k}$-structures corresponding to the different coherent orientations of the same underlying $k$-tree structure.

It remains to extend this action of $\symgp{k}$ to the root of a rooted $k$-coding tree.
The action on coding trees rooted at white vertices (that is, on the species $\pointed[Y]{\specname{CT}}_{k}$) is trivial in the same sense as for $F_{k}$; to apply a permutation $\sigma$ to a $\pointed[Y]{\specname{CT}}_{k}$-structure, simply apply it to each of the $H_{k}$-structures which is a child of the root white vertex and proceed recursively.
This gives an $\symgp{k}$-action on $\pointed[Y]{\specname{CT}}_{k}$-structures which commutes with permutations of labels, so we can take quotients.
Moreover, we can easily pass this action to the species $\pointed[Y]{\vec{\mathfrak{a}}}_{k}$ of coherently-oriented $k$-trees rooted at fronts, and its orbits are clearly the underlying unoriented front-rooted $k$-trees of $\pointed[Y]{\mathfrak{a}}_{k}$.
We can then conclude that:
\begin{theorem}
  \label{thm:ayquot}
  For $\pointed[Y]{\mathfrak{a}}_{k}$ the species of $k$-trees rooted at fronts and $\pointed[Y]{\specname{CT}}_{k}$ the species of $k$-coding trees rooted at white vertices, we have
  \begin{equation}
    \label{eq:ayquot}
    \pointed[Y]{\mathfrak{a}}_{k} = \faktor{\pointed[Y]{\specname{CT}}_{k}}{\symgp{k}}
  \end{equation}
  for the $\symgp{k}$-action described above.
\end{theorem}

Similarly, we may extend this $\symgp{k}$-action to the species $\pointed[XY]{\specname{CT}}_{k}$ of edge-rooted $k$-coding trees.
By equation \eqref{eq:ctxyfunc}, we can decompose a $\pointed[XY]{\specname{CT}}_{k}$-structure into an $F_{k}$-structure and an $H_{k}$-structure, each of which has a well-defined $\symgp{k}$-action, which we simply pass back up to $\pointed[XY]{\specname{CT}}_{k}$.
By a similar argument to that above, it is clear that orbits of $\pointed[XY]{\specname{CT}}_{k}$-structures are in natural bijection with $\pointed[XY]{\mathfrak{a}}_{k}$-structures, so:
\begin{theorem}
  \label{thm:axyquot}
  For $\pointed[XY]{\mathfrak{a}}_{k}$ the species of $k$-trees rooted at hedra with designated fronts, $\pointed[XY]{\specname{CT}}_{k}$ the species of $k$-coding trees rooted at edges, and $H_{k}$ and $F_{k}$ the component species of black- and white-vertex-rooted $k$-coding tree components, we have
  \begin{equation}
    \label{eq:axyquot}
    \pointed[XY]{\mathfrak{a}}_{k} = \faktor{\pointed[XY]{\specname{CT}}_{k}}{\symgp{k}}  = \pbrac{\faktor{H_{k}}{\symgp{k}}} \cdot \pbrac{\faktor{F_{k}}{\symgp{k}}}.
  \end{equation}
  for the $\symgp{k}$-action described above.
\end{theorem}
...
\subsection{Quotients for $\pointed[X]{\mathfrak{a}}_{k}$}\label{ss:axact}
We have thus far defined $\symgp{k}$-actions on the species $H_{k}$, $F_{k}$, $\pointed[Y]{\specname{CT}}_{k}$, and $\pointed[XY]{\specname{CT}}_{k}$, and used these to characterize the species $\pointed[Y]{\mathfrak{a}}_{k}$ and $\pointed[XY]{\mathfrak{a}}_{k}$.
However, to apply Theorem \ref{thm:dissymk} to study the species $\mathfrak{a}_{k}$ itself, we need to characterize the species $\pointed[X]{\mathfrak{a}}_{k}$ as well.
To do this, we need a result analogous to Theorem \ref{thm:ayquot} and Theorem \ref{thm:axyquot}.

Each $\pointed[X]{\mathfrak{a}}_{k}$-structure (that is, hedron-rooted $k$-tree) $T$ is naturally identified with the set of $\pbrac{\pointed[X]{\vec{\mathfrak{a}}}_{k} = \pointed[X]{\specname{CT}}_{k}}$-structures that are the coherent orientations of $T$.
However, as noted in the previous section, there is no apparent way to give this set as an orbit of an $\symgp{k}$-action on $\pointed[X]{\specname{CT}}_{k}$, since indeed there is no apparent such group action at all.
If, however, we identify each $\pointed[X]{\specname{CT}}_{k}$-structure $T$ with the set of $\pointed[XY]{\specname{CT}}_{k}$-structures which are obtained by rooting at all the edges incident to the root black vertex of $T$, we have an $\symgp{k}$-action on the latter set whose orbits are $\pointed[XY]{\mathfrak{a}}_{k}$-structures.
Moreover, if we write $C_{k+1}$ for the cyclic group with $k+1$ elements, there is a natural $C_{k+1}$-action on $\pointed[XY]{\specname{CT}}_{k}$-structures which rotates the choice of root white vertex around the root black vertex with respect to the cyclic order on those white neighbors.
We will write $C_{k+1} = \abrac{\zeta}$ where the action of $\zeta$ is to rotate the choice of root white vertex one position forward with respect to the cyclic order on the white neighbors of the black root of an edge-rooted $k$-coding tree.

These actions of $\symgp{k}$ and $C_{k+1}$ do not commute, so the combined group which acts on $\pointed[XY]{\specname{CT}}_{k}$ is not their direct product.
It is straightforward to show that it is fact a semidirect product $C_{k+1} \rtimes \symgp{k}$.
However, we will not need to use the algebraic structure, so we leave aside the details of the multiplication.
We note, however, that the set of elements of $C_{k+1} \rtimes \symgp{k+1}$ is simply the Cartesian product, so every element $\gamma$ has an expression $\gamma = \sigma \zeta^{i}$ which acts by cyclic rotation of the white root by $i$ positions followed by application of our standard action of the permutation $\sigma$ on the resulting structure.

If we treat the $\specname{C}_{k+1}$-structure on the white neighbors of the black root of a $\pointed[XY]{\specname{CT}}_{k}$-structure as an $\specname{L}_{k+1}$-structure (rather than the usual $\specname{L}_{k}$) by appending the white root to the \emph{end} of the linear order, there is a natural $\symgp{k+1}$-action on this linear order by permutations.
Let $i_{1}: \symgp{k} \hookrightarrow \symgp{k+1}$ be the map sending each $\sigma \in \symgp{k}$ to the permutation in $\symgp{k+1}$ which acts as $\sigma$ on $\cbrac{1, \dots, k}$ and fixes $k+1$; we will usually simply denote this permutation $i_{1} \pbrac{\sigma}$ by $\sigma$ again.
Similarly, let $i_{2}: C_{k+1} \hookrightarrow \symgp{k+1}$ be the map sending the generator $\zeta$ to the cyclic element $\pbrac{1\ 2\ \dots\ k+1}$; we will usually denote this permutation $i_{2} \pbrac{\zeta}$ by $\zeta$ as well.
These maps are monomorphisms, and furthermore they respect the actions of $\symgp{k}$ and $C_{k+1}$ on $\specname{L}_{k+1}$; an element from $\symgp{k}$ still permutes the linear order on the non-root white neighbors of the root black vertex, while an element from $C_{k+1}$ still preserves the underlying cyclic order on the white vertices but changes which one is root.
Thus, the pair $\zeta^{i} \in C_{k+1}$ and $\sigma \in \symgp{k}$ act on an $\specname{L}_{k+1}$-structure exactly as their image $\sigma \zeta^{i} \in \symgp{k+1}$ does.

Once some pair $\pbrac{\zeta^{i}, \sigma}$ is allowed to act on the $\specname{L}_{k+1}$-structure at the root of a $\pointed[XY]{\specname{CT}}_{k}$-structure, however, we must adjust the orderings of the white children of non-root black vertices to preserve the coherence of the orientation of the associated $k$-tree.
Permutations of the form $\rho_{j} \pbrac{\sigma}$ applied to each of the $F_{k}$-structures attached to the root black vertex will suffice once again, but now the $F_{k}$-structure which begins in position $j$ must undergo the action of $\rho_{\zeta^{i} \pbrac{j}} \pbrac{\sigma}$, since after the application of $\zeta^{i}$ it is in position $\zeta^{i} \pbrac{j}$.
This will preserve coherence as desired.

With the interaction of the actions of $C_{k+1}$ and $\symgp{k}$ on $\pointed[XY]{\specname{CT}}_{k}$-structures so defined, it is clear that each orbit of the combined action $C_{k+1} \rtimes \symgp{k}$ corresponds to the set of all the ways to take a given hedron-rooted $k$-tree, select a front of the root hedron, and coherently orient the result.
Thus:
\begin{theorem}
  \label{thm:axquot}
  For $\pointed[X]{\mathfrak{a}}_{k}$ the species of $k$-trees rooted at hedra and $\pointed[XY]{\specname{CT}}_{k}$ the species of $k$-coding trees rooted at edges, we have
  \begin{equation}
    \label{eq:axquot}
    \pointed[X]{\mathfrak{a}}_{k} = \faktor{\pointed[XY]{\specname{CT}}_{k}}{\pbrac{C_{k+1} \rtimes \symgp{k}}}
  \end{equation}
  for the action of $C_{k+1} \rtimes \symgp{k}$ described above.
\end{theorem}

We now have all the pieces of the original dissymmetry Theorem \ref{thm:dissymk} save $\mathfrak{a}_{k}$ itself characterized in terms of quotients of species of rooted $k$-coding trees: $\pointed[Y]{\mathfrak{a}}_{k}$ in \ref{thm:ayquot}, $\pointed[XY]{\mathfrak{a}}_{k}$ in \ref{thm:axyquot}, and $\pointed[X]{\mathfrak{a}}_{k}$ in \ref{thm:axquot}.
...
Corollary \ref{cor:dissymkreform} has a direct analogue in terms of cycle indices:
\begin{theorem}
  \label{thm:dissymkci}
  For the various forms of the species $\mathfrak{a}_{k}$ as in \S \ref{ss:dissymk}, we have
  \begin{equation}
    \label{eq:dissymkci}
    \gci{\symgp{k}}{\mathfrak{a}_{k}} = \gci{\symgp{k}}{\pointed[X]{\mathfrak{a}}_{k}} + \gci{\symgp{k}}{\pointed[Y]{\mathfrak{a}}_{k}} - \gci{\symgp{k}}{\pointed[XY]{\mathfrak{a}}_{k}}.
  \end{equation}
\end{theorem}
Thus, we need to calculate the cycle indices of the three rooted forms of $\mathfrak{a}_{k}$.
From Theorems \ref{thm:ayquot}, \ref{thm:axyquot}, and \ref{thm:axquot}, we obtain:
\begin{theorem}
  \label{thm:aquotci}
  For the various forms of the species $\mathfrak{a}_{k}$ as in \S \ref{ss:dissymk} and the various $\symgp{k}$-species and $\pbrac{C_{k+1} \rtimes \symgp{k}}$-species forms of $\specname{CT}_{k}$ as in \S \ref{s:genkt}, we have
  \begin{subequations}
    \label{eq:aquotci}
    \begin{align}
      \gci{\symgp{k}}{\pointed[Y]{\mathfrak{a}}_{k}} &= \frac{1}{k!} \sum_{\sigma \in \symgp{k}} \gci{\symgp{k}}{\pointed[Y]{\specname{CT}}_{k}} \pbrac{\sigma} \label{eq:ayquotci} \\
      \gci{\symgp{k}}{\pointed[XY]{\mathfrak{a}}_{k}} &= \frac{1}{k!} \sum_{\sigma \in \symgp{k}} \gci{\symgp{k}}{\pointed[XY]{\specname{CT}}_{k}} \pbrac{\sigma} = \frac{1}{k!} \sum_{\sigma \in \symgp{k}} \pbrac{\gci{\symgp{k}}{F_{k}} \pbrac{\sigma} \cdot \gci{\symgp{k}}{H_{k}} \pbrac{\sigma}} \label{eq:axyquotci} \\
      \gci{\symgp{k}}{\pointed[X]{\mathfrak{a}}_{k}} &= \frac{1}{k! \pbrac{k+1}} \sum_{\substack{\sigma \in \symgp{k} \\ i \in \cbrac{0, 1, \dots, k}}} \gci{\symgp{k}}{\pointed[XY]{\specname{CT}}_{k}} \pbrac{\sigma, \zeta^{i}} \label{eq:axquotci}
    \end{align}
  \end{subequations}
\end{theorem}

We thus need only calculate the various $\Gamma$-cycle indices for the three rooted forms of the species $\specname{CT}_{k}$ to complete our enumeration of general $k$-trees.

\subsection{Recursive components: $H_{k}$ and $F_{k}$}\label{ss:ktcycindrec}
We first consider the component $\symgp{k}$-species $F_{k}$ and $H_{k}$, first defined in \S \ref{ss:codecomp}, which we saw in that section are critical to understanding $k$-coding trees.

We see in Theorem \ref{thm:funcdecompct} that $F_{k} = Y \cdot \specname{E} \pbrac{H_{k}}$ as ordinary species---that is, that an $F_{k}$-structure is composed of a $Y$-label and a set of $H_{k}$-structures.
More specifically, an $F_{k}$-structure for which some $\sigma \in \symgp{k}$ is an automorphism is in fact still just a $Y$-labeled root and a set of $H_{k}$-structures for which that same $\sigma$ is an automorphism.
Thus, equation \eqref{eq:ffunc} holds in $\symgp{k}$-generality, and we conclude:
\begin{equation}
  \label{eq:ffuncci}
  \gci{\symgp{k}}{F_{k}} \pbrac{\sigma} = p_{1} \sbrac{y} \cdot \pbrac{\ci{\specname{E}'} \circ \gci{\symgp{k}}{H_{k}} \pbrac{\sigma}}.
\end{equation}

In Theorem \ref{thm:funcdecompct}, we also have that $H_{k} = X \cdot \specname{L}_{k+1} \pbrac{F_{k}}$ as ordinary species.
However, this does \emph{not} hold as an identity of $\symgp{k}$-species; an $H_{k}$-structure which for which some $\sigma \in \symgp{k}$ is an automorphism is composed of an $X$-label and a linearly-ordered set (of cardinality $k+1$) of $F_{k}$-structures, but Theorem \ref{thm:rhodef} implies that these $F_{k}$ structures must satisfy a complicated condition in terms of the $\rho_{i} \pbrac{\sigma}$'s.
\begin{lemma}
  \label{lem:fkinvar}
  Let $H$ be a structure of the species $H_{k} = X \cdot \specname{L}_{k+1} \pbrac{F_{k}}$.
  Let $F \pbrac{i}$ be the $F_{k}$-structure in the $i$th position in the linear order.
  Then some $\sigma \in \symgp{k}$ acts as an automorphism of $H$ if and only if, for each $i \in \sbrac{k+1}$, we have $\rho_{i} \pbrac{\sigma} F_{i} \cong F_{\sigma \pbrac{i}}$.
\end{lemma}

\begin{proof}
  The $X$-label on the black root of $H$ is not affected by the application of $\sigma$, so it imposes no conditions.
  However, the $\specname{L}_{k+1}$-structure on the white children of the root is permuted by $\sigma$, and we apply to each of the $F \pbrac{i}$'s the action of $\rho_{i} \pbrac{\sigma}$.
  Thus, $\sigma$ is an automorphism of $H$ if and only if the combination of applying $\sigma$ to the linear order and $\rho_{i} \pbrac{\sigma}$ to each $F_{i}$ is an automorphism.
  Since $\sigma$ `carries' each $F_{i}$ onto $F_{\sigma \pbrac{i}}$, we must have that $\rho_{i} \pbrac{\sigma} F_{i} \cong F_{\sigma \pbrac{i}}$, as desired.
  That this suffices is clear.
\end{proof}

With this result, we can calculate the $\symgp{k}$-cycle index of $H_{k}$:
\begin{theorem}
  \label{thm:hfuncci}
  The $\symgp{k}$-cycle index for the species $H_{k}$ is given by
  \begin{multline}
    \label{eq:hfuncci}
    \gci{\symgp{k}}{H_{k}} \pbrac{\sigma} = p_{1} \sbrac{x} \\
    \times \prod_{c \in C \pbrac{\sigma}} \gci{\symgp{k}}{F_{k}} \pbrac{\prod_{i \in c} \rho_{i} \pbrac{\sigma}} \sbrac{p_{\abs{c}} \sbrac{x}, p_{2 \abs{c}} \sbrac{x}, \dots; p_{\abs{c}} \sbrac{y}, p_{2 \abs{c}} \sbrac{y}, \dots}.
  \end{multline}
  where $C \pbrac{\sigma}$ denotes the set of cycles of $\sigma$ and where the inner product is taken over the elements $i$ of the cycle $c$ in the cyclic order starting from any element.
\end{theorem}

\begin{proof}
  By an obvious multisort extension of Definition \ref{def:gcycind}, for each $\sigma \in \symgp{k}$ and $\gamma \in \symgp{n} \times \symgp{kn + 1}$, we wish to characterize the $H_{k}$-structures which are fixed by $\sigma \circ \gamma$.
  For any given $\sigma \in \symgp{k}$ which induces an automorphism of a given $H_{k}$-structure $H$, there is exactly one $\gamma \in \symgp{k} \times \symgp{kn+1}$ such that $\sigma \circ \gamma$ fixes $H$---namely, the $\gamma$ which returns each of the labels of $H$ to their original places after $\sigma$ is applied.
  Similarly, if $\sigma$ does \emph{not} induce an automorphism of $H$, there can be no such $\gamma$.
  Define $\fix H_{k} \sbrac{\sigma}$ to be the number of $H_{k}$-structures with $n$ black vertices for which $\sigma$ induces an automorphism and let $p_{\sigma}$ denote $p_{\gamma}$ for $\gamma$ the permutation on labels which is specified by $\sigma$.
  We then wish to calculate $\fix H_{k} \sbrac{\sigma}$ for each $\sigma \in \symgp{k}$.

  As we have noted, the $X$-label on the root black vertex is unaffected by the action of every $\sigma$, so there must be a $p_{1} \sbrac{x}$ term in the result.
  From Lemma \ref{lem:fkinvar} it is apparent that each cycle of $\sigma$ can be considered independently.
  Let $c = \pbrac{c_{1} c_{2} \dots c_{l}}$ be such a cycle (that is, $\sigma \pbrac{c_{i}} = c_{i+1}$, allowing that $l+1 = 1$ cyclically).
  Let the $F_{k}$-structure at position $c_{1}$ be denoted $F$.
  Then the $F_{k}$-structure at position $c_{2}$ must be isomorphic to $\rho_{c_{1}} \pbrac{\sigma} F$, that at position $c_{3}$ must be isomorphic to $\rho_{c_{2}} \pbrac{\sigma} \pbrac{\rho_{c_{1}} \pbrac{\sigma} F} = \pbrac{\rho_{c_{2}} \cdot \rho_{c_{1}}} \pbrac{\sigma} F$, and so on.
  Thus, up to permutations of their labels, all the $F_{k}$ structures along $c$ are determined by $F$.
  Moreover, since $c$ is a cycle, $F$ must come back to itself after $l$ iterations of $\sigma$.

  Let $\psi = \prod_{i = 1}^{l} \rho_{c_{i}} \pbrac{\sigma}$; then $\psi$ must induce an automorphism of $F$.
  By lemma \ref{lem:fkinvar}, this requirement suffices.
  Therefore, with respect to the cycle $c$, we have $\fix H_{k} \sbrac{\psi}$ choices for the structure $F$, and the label permutation $\gamma$ which is induced by this isomorphism has an $l$-cycle for every $1$-cycle of $\psi$, a $2l$-cycle for every $2$-cycle of $\psi$, and so on.
  The contribution to the cycle index is then $\fix H_{k} \sbrac{\psi} p_{\psi}^{l}$, which is exactly the term  \begin{equation*}
    \gci{\symgp{k}}{F_{k}} \pbrac{\prod_{i \in c} \rho_{i} \pbrac{\sigma}} \sbrac{p_{\abs{c}} \sbrac{x}, p_{2 \abs{c}} \sbrac{x}, \dots; p_{\abs{c}} \sbrac{y}, p_{2 \abs{c}} \sbrac{y}, \dots}
  \end{equation*}
  in equation \eqref{eq:hfuncci}.
\end{proof}

\subsection{Rooted components: $\pointed[Y]{\specname{CT}}_{k}$ and $\pointed[XY]{\specname{CT}}_{k}$}\label{ss:ktcycindroot}
We now consider the $\symgp{k}$-species $\pointed[Y]{\specname{CT}}_{k}$ of $k$-coding trees rooted at white vertices and the $\pbrac{C_{k+1} \rtimes \symgp{k}}$-species $\pointed[XY]{\specname{CT}}_{k}$ of $k$-coding trees rooted at edges.
As in the previous section, the action of $\sigma \in \symgp{k}$ on the $\specname{E}$-structure in equation \eqref{eq:ctyfunc} is trivial, so again species composition is the desired operation:
\begin{equation}
  \label{eq:ctyfuncci}
  \gci{\symgp{k}}{\pointed[Y]{\specname{CT}}_{k}} \pbrac{\sigma} = p_{1} \sbrac{y} \cdot \pbrac{\ci{\specname{E}} \circ \gci{\symgp{k}}{H_{k}} \pbrac{\sigma}}.
\end{equation}

However, as seen in \S \ref{ss:axact}, the complexities of the species $H_{k}$ (as in Theorem \ref{thm:hfuncci}) return, with slight modifications:
\begin{theorem}
  \label{thm:ctxyfuncci}
  The $\pbrac{\symgp{k} \times C_{k+1}}$-cycle index for the species $\pointed[XY]{\specname{CT}}_{k}$ is given by
  \begin{multline}
    \label{eq:ctxyfuncci}
    \gci{\symgp{k} \rtimes C_{k+1}}{\pointed[XY]{\specname{CT}}_{k}} \pbrac{\sigma, \zeta^{j}} = p_{1} \sbrac{x} \\
    \times \prod_{c \in C \pbrac{\sigma \zeta^{j}}} \gci{\symgp{k}}{F_{k}} \pbrac{\prod_{i \in c} \rho_{\zeta^{j} \pbrac{i}} \sbrac{\sigma}} \sbrac{p_{\abs{c}} \sbrac{x}, p_{2 \abs{c}} \sbrac{x}, \dots, p_{\abs{c}} \sbrac{y}, p_{2 \abs{c}} \sbrac{y}, \dots}.
  \end{multline}
  under the same conditions as Theorem \ref{thm:hfuncci}.
\end{theorem}

\begin{proof}
  The proof is essentially identical to that of Theorem \ref{thm:hfuncci}.
\end{proof}
...
We substitute the results of the previous two sections into equation \eqref{eq:dissymkci} to obtain:
\begin{theorem}[Cycle index for the species of $k$-trees]
  For $\gci{\symgp{k}}{\pointed[Y]{\specname{CT}}_{k}}$ as in equation \eqref{eq:ctyfuncci}, $\gci{\symgp{k} \rtimes C_{k+1}}{\pointed[XY]{\specname{CT}}_{k}}$ as in equation \eqref{eq:ctxyfuncci}, $\gci{\symgp{k}}{H_{k}}$ as in equation \eqref{eq:hfuncci}, and $\gci{\symgp{k}}{F_{k}}$ as in equation \eqref{eq:ffuncci}, we have:
  \label{thm:ktreecyc}
  \begin{multline}
    \quad \gci{\symgp{k}}{\mathfrak{a}_{k}} = \frac{1}{k! \pbrac{k+1}} \sum_{\substack{\sigma \in \symgp{k} \\ i \in \cbrac{0, 1, \dots, k}}} \gci{\symgp{k} \rtimes C_{k+1}}{\pointed[XY]{\specname{CT}}_{k}} \pbrac{\sigma, \zeta^{i}} \\
    + \frac{1}{k!} \sum_{\sigma \in \symgp{k}} \gci{\symgp{k}}{\pointed[Y]{\specname{CT}}_{k}} \pbrac{\sigma} - \frac{1}{k!} \sum_{\sigma \in \symgp{k}} \pbrac{\gci{\symgp{k}}{F_{k}} \pbrac{\sigma} \cdot \gci{\symgp{k}}{H_{k}} \pbrac{\sigma}}. \quad
  \end{multline}
\end{theorem}

Since the formulas for $\gci{\symgp{k}}{F_{k}}$ and $\gci{\symgp{k}}{H_{k}}$ depend on each other, this result in fact represents a recursive system of functional equations.
