...
More specifically, an $F_{k}$-structure for which some $\sigma \in \mathfrak{S}_{k}$ is an automorphism in fact still just a $Y$-label and a set of $H_{k}$-structures for which that same $\sigma$ is an automorphism.
...
However, this does \emph{not} hold as an identity of $S_{k}$-species; an $H_{k}$-structure which for which some $\sigma \in \mathfrak{S}_{k}$ is an automorphism is composed of an $X$-label and a linearly-ordered set (of cardinality $k+1$) of $F_{k}$-structures, but Theorem \ref{thm:rhodef} implies that these $F_{k}$ structures must satisfy a complicated condition in terms of the $\rho_{i} \pbrac{\sigma}$'s.
\begin{lemma}
  \label{lem:fkinvar}
...
\begin{proof}
  ...
  We then wish to calculate $\fix H_{k} \sbrac{\sigma} p_{\sigma}$ for each $\sigma \in \mathfrak{S}_{k}$.
...
\begin{proof}
  Proof is essentially identical to that of Theorem \ref{thm:hfuncci}.
\end{proof}
