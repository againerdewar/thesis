...
\begin{definition}
  ...
  \begin{equation}
    \label{eq:cycinddef}
    Z_{F} \pbrac{p_{1}, p_{2}, \dots} := \sum_{n \geq 0} \frac{1}{n!} \big( \sum_{\sigma \in \mathfrak{S}_{n}} \fix \pbrac{F \sbrac{\sigma}} p_{1}^{\sigma_{1}} p_{2}^{\sigma_{2}} \dots \big) = \sum_{n \geq 0} \frac{1}{n!} \big( \sum_{\sigma \in \mathfrak{S}_{n}} \fix \pbrac{F \sbrac{\sigma}} p_{\sigma} \big)
  \end{equation}
...
In fact, $\fix \pbrac{F \sbrac{\sigma}}$ is a class function\footnote{That is, the value of $\fix \pbrac{F \sbrac{\sigma}}$ will be constant on conjugacy classes of permutations, which we note are exactly the sets of permutations of fixed cycle type.} on permutations $\sigma \in \mathfrak{S}_{n}$.
...
\begin{multline}
  ...
  \sum_{\substack{n \geq 0 \\ a_{1} + a_{2} + \dots + a_{k} = n}} \frac{1}{a_{1}! a_{2}! \dots a_{k}!} \sum_{\sigma \in \mathfrak{S}_{a_{1}, a_{2}, \dots, a_{k}}} \fix \pbrac{F \sbrac{\sigma}} p^{\sigma_{1}}_{\sbrac{1}} p^{\sigma_{2}}_{\sbrac{2}} \dots p^{\sigma_{k}}_{\sbrac{k}}.
\end{multline}
...
\begin{definition}
  ...
  \begin{equation}
    Z_{F} \pbrac{\gamma} = \sum_{n \geq 0} \frac{1}{n!} \sum_{\sigma \in \mathfrak{S}_{n}} \fix \pbrac{\gamma \cdot F \sbrac{\sigma}} p_{\sigma} \label{eq:gcycinddef}
  \end{equation}
...
Recall from equation \eqref{eq:cycinddef} that, to compute the cycle index of a species, we need to enumerate the fixed points of each $\sigma \in \mathfrak{S}_{n}$.
...
\begin{theorem}\label{thm:qsci}
  ...
  \begin{equation}
    \label{eq:quotcycind}
    Z_{F / \Gamma} = \frac{1}{\abs{\Gamma}} \sum_{\gamma \in \Gamma} Z_{F} \pbrac{\gamma} = \frac{1}{\abs{\Gamma}} \sum_{\substack{n \geq 0 \\ \sigma \in \mathfrak{S}_{n} \\ \gamma \in \Gamma}} \frac{1}{n!} \pbrac{\gamma \cdot F \sbrac{\sigma}} p_{\sigma}.
  \end{equation}
\end{theorem}
...
\begin{equation*}
  Z_{F} \pbrac{\gamma} = \sum_{n \geq 0} \frac{1}{n!} \sum_{\sigma \in \mathfrak{S}_{n}} \fix \pbrac{\gamma \cdot F \sbrac{\sigma}} p_{\sigma}.
\end{equation*}
...
\begin{lemma}
  \label{lem:notransac}
  For $k \geq 3$, no transitive action of any group on the set $\cyc_{\sbrac{k+1}}$ of cyclic orders on $\sbrac{k+1}$ commutes with the action of $\mathfrak{S}_{k+1}$ that permutes labels.
\end{lemma}
\begin{proof}
  We represent the elements of $\cyc_{\sbrac{k+1}}$ as cyclic permutations on the alphabet $\sbrac{k+1}$; then the action of $\mathfrak{S}_{k+1}$ that permutes labels is exactly the conjugation action on these permutations.
...
\begin{theorem}
  \label{thm:rhodef}
  If a permutation $\sigma \in \mathfrak{S}_{k}$ is applied to the orientation of the root black vertex in an $H_{k}$-structure, the permutation which must be applied to a child black vertex which was attached to the $i$th white vertex (with respect to the linear ordering induced by the orientation) to preserve coherence is $\rho_{i} \pbrac{\sigma}$, where $\rho_{i}$ is the map
  \begin{equation}
    \label{eq:rhodef}
...
This yields a well-defined $\mathfrak{S}_{k}$-action on the species $H_{k}$ which, we note, is completely label-independent and thus naturally commutes with the $\pbrac{\mathfrak{S}_{n} \times \mathfrak{S}_{kn+1}}$-action on labels, as we might hope in order to apply the theory of $\Gamma$-species.
...
This gives an $\mathfrak{S}_{k}$-action on $\pointed[Y]{\specname{CT}}_{k}$-structures which commutes with permutations of labels, so we can take quotients.
...
\begin{theorem}
  ...
  \begin{equation}
    \label{eq:ayquot}
    \pointed[Y]{\mathfrak{a}}_{k} = \faktor{\pointed[Y]{\specname{CT}}_{k}}{\mathfrak{S}_{k}}
  \end{equation}
  for the $\mathfrak{S}_{k}$-action described above.
\end{theorem}

Similarly, we may extend this $\mathfrak{S}_{k}$-action to the species $\pointed[XY]{\specname{CT}}_{k}$ of edge-rooted $k$-coding trees.
By equation \eqref{eq:ctxyfunc}, we can decompose a $\pointed[XY]{\specname{CT}}_{k}$-structure into an $F_{k}$-structure and an $H_{k}$-structure, each of which has a well-defined $\mathfrak{S}_{k}$-action, which we simply pass back up to $\pointed[XY]{\specname{CT}}_{k}$.
...
\begin{theorem}
  ...
  \begin{equation}
    \label{eq:axyquot}
    \pointed[XY]{\mathfrak{a}}_{k} = \faktor{\pointed[XY]{\specname{CT}}_{k}}{\mathfrak{S}_{k}}  = \pbrac{\faktor{H_{k}}{\mathfrak{S}_{k}}} \cdot \pbrac{\faktor{F_{k}}{\mathfrak{S}_{k}}}.
  \end{equation}
  for the $\mathfrak{S}_{k}$-action described above.
\end{theorem}
...
We have thus far defined $\mathfrak{S}_{k}$-actions on the species $H_{k}$, $F_{k}$, $\pointed[Y]{\specname{CT}}_{k}$, and $\pointed[XY]{\specname{CT}}_{k}$, and used these to characterize the species $\pointed[Y]{\mathfrak{a}}_{k}$ and $\pointed[XY]{\mathfrak{a}}_{k}$.
...
However, as noted in the previous section, there is no apparent way to give this set as an orbit of an $\mathfrak{S}_{k}$-action on $\pointed[X]{\specname{CT}}_{k}$, since indeed there is no apparent such group action at all.
If, however, we identify each $\pointed[X]{\specname{CT}}_{k}$-structure $T$ with the set of $\pointed[XY]{\specname{CT}}_{k}$-structures which are obtained by rooting at all the edges incident to the root black vertex of $T$, we have an $\mathfrak{S}_{k}$-action on the latter set whose orbits are $\pointed[XY]{\mathfrak{a}}_{k}$-structures.
...
These actions of $\mathfrak{S}_{k}$ and $C_{k+1}$ do not commute, so the combined group which acts on $\pointed[XY]{\specname{CT}}_{k}$ is not their direct product.
It is straightforward to show that it is fact a semidirect product $C_{k+1} \rtimes \mathfrak{S}_{k}$.
However, we will not need to use the algebraic structure, so we leave aside the details of the multiplication.
We note, however, that the set of elements of $C_{k+1} \rtimes \mathfrak{S}_{k+1}$ is simply the Cartesian product, so every element $\gamma$ has an expression $\gamma = \sigma \zeta^{i}$ which acts by cyclic rotation of the white root by $i$ positions followed by application of our standard action of the permutation $\sigma$ on the resulting structure.

If we treat the $\specname{C}_{k+1}$-structure on the white neighbors of the black root of a $\pointed[XY]{\specname{CT}}_{k}$-structure as an $\specname{L}_{k+1}$-structure (rather than the usual $\specname{L}_{k}$) by appending the white root to the \emph{end} of the linear order, there is a natural $\mathfrak{S}_{k+1}$-action on this linear order by permutations.
Let $i_{1}: \mathfrak{S}_{k} \hookrightarrow \mathfrak{S}_{k+1}$ be the map sending each $\sigma \in \mathfrak{S}_{k}$ to the permutation in $\mathfrak{S}_{k+1}$ which acts as $\sigma$ on $\cbrac{1, \dots, k}$ and fixes $k+1$; we will usually simply denote this permutation $i_{1} \pbrac{\sigma}$ by $\sigma$ again.
Similarly, let $i_{2}: C_{k+1} \hookrightarrow \mathfrak{S}_{k+1}$ be the map sending the generator $\zeta$ to the cyclic element $\pbrac{1\ 2\ \dots\ k+1}$; we will usually denote this permutation $i_{2} \pbrac{\zeta}$ by $\zeta$ as well.
These maps are monomorphisms, and furthermore they respect the actions of $\mathfrak{S}_{k}$ and $C_{k+1}$ on $\specname{L}_{k+1}$; an element from $\mathfrak{S}_{k}$ still permutes the linear order on the non-root white neighbors of the root black vertex, while an element from $C_{k+1}$ still preserves the underlying cyclic order on the white vertices but changes which one is root.
Thus, the pair $\zeta^{i} \in C_{k+1}$ and $\sigma \in \mathfrak{S}_{k}$ act on an $\specname{L}_{k+1}$-structure exactly as their image $\sigma \zeta^{i} \in \mathfrak{S}_{k+1}$ does.
...
With the interaction of the actions of $C_{k+1}$ and $\mathfrak{S}_{k}$ on $\pointed[XY]{\specname{CT}}_{k}$-structures so defined, it is clear that each orbit of the combined action $C_{k+1} \rtimes \mathfrak{S}_{k}$ corresponds to the set of all the ways to take a given hedron-rooted $k$-tree, select a front of the root hedron, and coherently orient the result.
Thus:
\begin{theorem}
  \label{thm:axquot}
  ...
  \begin{equation}
    \label{eq:axquot}
    \pointed[X]{\mathfrak{a}}_{k} = \faktor{\pointed[XY]{\specname{CT}}_{k}}{\pbrac{C_{k+1} \rtimes \mathfrak{S}_{k}}}
  \end{equation}
  for the action of $C_{k+1} \rtimes \mathfrak{S}_{k}$ described above.
\end{theorem}
...
\begin{theorem}
  ...
  \begin{equation}
    \label{eq:dissymkci}
    Z_{\mathfrak{a}_{k}} = Z_{\pointed[X]{\mathfrak{a}}_{k}} + Z_{\pointed[Y]{\mathfrak{a}}_{k}} - Z_{\pointed[XY]{\mathfrak{a}}_{k}}.
  \end{equation}
\end{theorem}
...
\begin{theorem}
  \label{thm:aquotci}
  For the various forms of the species $\mathfrak{a}_{k}$ as in \S \ref{ss:dissymk} and the various $\mathfrak{S}_{k}$-species and $\pbrac{C_{k+1} \rtimes \mathfrak{S}_{k}}$-species forms of $\specname{CT}_{k}$ as in \S \ref{s:genkt}, we have
  \begin{subequations}
    \label{eq:aquotci}
    \begin{align}
      Z_{\pointed[Y]{\mathfrak{a}}_{k}} &= \frac{1}{k!} \sum_{\sigma \in \mathfrak{S}_{k}} Z_{\pointed[Y]{\specname{CT}}_{k}} \pbrac{\sigma} \label{eq:ayquotci} \\
      Z_{\pointed[XY]{\mathfrak{a}}_{k}} &= \frac{1}{k!} \sum_{\sigma \in \mathfrak{S}_{k}} Z_{\pointed[XY]{\specname{CT}}_{k}} \pbrac{\sigma} = \frac{1}{k!} \sum_{\sigma \in \mathfrak{S}_{k}} \pbrac{Z_{F_{k}} \pbrac{\sigma} \cdot Z_{H_{k}} \pbrac{\sigma}} \label{eq:axyquotci} \\
      Z_{\pointed[X]{\mathfrak{a}}_{k}} &= \frac{1}{k! \pbrac{k+1}} \sum_{\substack{\sigma \in \mathfrak{S}_{k} \\ i \in \cbrac{0, 1, \dots, k}}} Z_{\pointed[XY]{\specname{CT}}_{k}} \pbrac{\sigma, \zeta^{i}} \label{eq:axquotci}
    \end{align}
  \end{subequations}
\end{theorem}
...
We first consider the component $\mathfrak{S}_{k}$-species $F_{k}$ and $H_{k}$, first defined in \S \ref{ss:codecomp}, which we saw in that section are critical to understanding $k$-coding trees.
...
More specifically, an $F_{k}$-structure for which some $\sigma \in \mathfrak{S}_{k}$ is an automorphism is in fact still just a $Y$-labeled root and a set of $H_{k}$-structures for which that same $\sigma$ is an automorphism.
Thus, equation \eqref{eq:ffunc} holds in $\mathfrak{S}_{k}$-generality, and we conclude:
\begin{equation}
  \label{eq:ffuncci}
...
However, this does \emph{not} hold as an identity of $\mathfrak{S}_{k}$-species; an $H_{k}$-structure which for which some $\sigma \in \mathfrak{S}_{k}$ is an automorphism is composed of an $X$-label and a linearly-ordered set (of cardinality $k+1$) of $F_{k}$-structures, but Theorem \ref{thm:rhodef} implies that these $F_{k}$ structures must satisfy a complicated condition in terms of the $\rho_{i} \pbrac{\sigma}$'s.
\begin{lemma}
  \label{lem:fkinvar}
  ...
  Then some $\sigma \in \mathfrak{S}_{k}$ acts as an automorphism of $H$ if and only if, for each $i \in \sbrac{k+1}$, we have $\rho_{i} \pbrac{\sigma} F_{i} \cong F_{\sigma \pbrac{i}}$.
\end{lemma}
...
With this result, we can calculate the $\mathfrak{S}_{k}$-cycle index of $H_{k}$:
\begin{theorem}
  \label{thm:hfuncci}
...
\begin{proof}
  By an obvious multisort extension of Definition \ref{def:gcycind}, for each $\sigma \in \mathfrak{S}_{k}$ and $\gamma \in \mathfrak{S}_{n} \times \mathfrak{S}_{kn + 1}$, we wish to characterize the $H_{k}$-structures which are fixed by $\sigma \circ \gamma$.
  For any given $\sigma \in \mathfrak{S}_{k}$ which induces an automorphism of a given $H_{k}$-structure $H$, there is exactly one $\gamma \in \mathfrak{S}_{k} \times \mathfrak{S}_{kn+1}$ such that $\sigma \circ \gamma$ fixes $H$---namely, the $\gamma$ which returns each of the labels of $H$ to their original places after $\sigma$ is applied.
  ...
  We then wish to calculate $\fix H_{k} \sbrac{\sigma}$ for each $\sigma \in \mathfrak{S}_{k}$.
...
We now consider the $\mathfrak{S}_{k}$-species $\pointed[Y]{\specname{CT}}_{k}$ of $k$-coding trees rooted at white vertices and the $\pbrac{C_{k+1} \rtimes \mathfrak{S}_{k}}$-species $\pointed[XY]{\specname{CT}}_{k}$ of $k$-coding trees rooted at edges.
As in the previous section, the action of $\sigma \in \mathfrak{S}_{k}$ on the $\specname{E}$-structure in equation \eqref{eq:ctyfunc} is trivial, so again species composition is the desired operation:
\begin{equation}
  \label{eq:ctyfuncci}
  Z_{\pointed[Y]{\specname{CT}}_{k}} \pbrac{\sigma} = p_{1} \sbrac{y} \cdot \pbrac{Z_{\specname{E}} \circ Z_{H_{k}} \pbrac{\sigma}}.
\end{equation}
...
\begin{theorem}
  ...
  The $\pbrac{\mathfrak{S}_{k} \times C_{k+1}}$-cycle index for the species $\pointed[XY]{\specname{CT}}_{k}$ is given by
  \begin{multline}
    \label{eq:ctxyfuncci}
    Z_{\pointed[XY]{\specname{CT}}_{k}} \pbrac{\sigma, \zeta^{j}} = p_{1} \sbrac{x} \\
    \times \prod_{c \in C \pbrac{\sigma \zeta^{j}}} Z_{F_{k}} \pbrac{\prod_{i \in c} \rho_{\zeta^{j} \pbrac{i}} \sbrac{\sigma}} \sbrac{p_{\abs{c}} \sbrac{x}, p_{2 \abs{c}} \sbrac{x}, \dots, p_{\abs{c}} \sbrac{y}, p_{2 \abs{c}} \sbrac{y}, \dots}.
  \end{multline}
...
\begin{theorem}[Cycle index for the species of $k$-trees]
  For $Z_{\pointed[Y]{\specname{CT}}_{k}}$ as in equation \eqref{eq:ctyfuncci}, $Z_{\pointed[XY]{\specname{CT}}_{k}}$ as in equation \eqref{eq:ctxyfuncci}, $Z_{H_{k}}$ as in equation \eqref{eq:hfuncci}, and $Z_{F_{k}}$ as in equation \eqref{eq:ffuncci}, we have:
  \label{thm:ktreecyc}
  \begin{multline}
    \quad Z_{\mathfrak{a}_{k}} = \frac{1}{k! \pbrac{k+1}} \sum_{\substack{\sigma \in \mathfrak{S}_{k} \\ i \in \cbrac{0, 1, \dots, k}}} Z_{\pointed[XY]{\specname{CT}}_{k}} \pbrac{\sigma, \zeta^{i}} \\
    + \frac{1}{k!} \sum_{\sigma \in \mathfrak{S}_{k}} Z_{\pointed[Y]{\specname{CT}}_{k}} \pbrac{\sigma} - \frac{1}{k!} \sum_{\sigma \in \mathfrak{S}_{k}} \pbrac{Z_{F_{k}} \pbrac{\sigma} \cdot Z_{H_{k}} \pbrac{\sigma}}. \quad
  \end{multline}
\end{theorem}

Since the formulas for $Z_{F_{k}}$ and $Z_{H_{k}}$ depend on each other, this result in fact represents a recursive system of functional equations.
