\documentclass[sectionflow,singlespace,twoside]{brandiss} % Compactify for drafting (do *not* use these options for final submission!)
...
\end{definition}
In other words, the composition $F \circ G$ produces the species of $F$-structures of collections of $G$-structures. This definition is, again, partially motivated by the following result on cycle indices:
\begin{theorem}
...
We can then define a new species $\nicefrac{F}{\Gamma}$, the \emph{quotient species} of $F$ under the action of $\Gamma$, to be the species of $\Gamma$-orbits of
$F$-structures.
Moreover, for any bijection $\sigma: A \to B$, we then have an associated bijection $\nicefrac{F}{\Gamma} \sbrac{\sigma}: \nicefrac{F}{\Gamma} \sbrac{A} \to \nicefrac{F}{\Gamma} \sbrac{B}$, and the whole diagram
\begin{equation*}
  \begin{tikzpicture}[every node/.style={auto}]
    \matrix (m) [matrix of math nodes, row sep=4em, column sep=4em]
    {
    ...
    (m-1-2) edge node[below] {$\Gamma \curvearrowright$} (m-1-3)
    edge node {$F \sbrac{\sigma}$} (m-2-2)
    (m-1-3) edge node {$\nicefrac{F}{\Gamma} \sbrac{\sigma}$} (m-2-3)
    (m-2-1) edge node {$F$} (m-2-2)
    (m-2-2) edge node {$\Gamma \curvearrowright$} (m-2-3);
...
If instead we adopt the more common definition of a species as a functor $F: \mathbf{B} \to \mathbf{B}$, the maps denoted ``$\Gamma \curvearrowright$'' above are not in the target category $\mathbf{B}$, so passage to the $\Gamma$-quotient is no longer a natural transformation.
...
\begin{definition}\label{def:gpspleth}
  ...
  \end{equation}
\end{definition}
This definition of $\Gamma$-power series plethysm is then indeed the correct operation to pair with the composition of $\Gamma$-species:
